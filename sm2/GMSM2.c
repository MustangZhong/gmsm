...
int SM2_Encrypt_New(unsigned char *data, int data_len, unsigned char *pub_key_x, unsigned char *pub_key_y, unsigned char *cipher)
{
    epoint      *kG;
    big     x, y;
    unsigned char * p_x_char_out = NULL;
    unsigned char * p_y_char_out = NULL;
/* standard data */
    unsigned char   std_rand[32] = { 0x60, 0x27, 0x6E, 0x27, 0xD5, 0x06, 0x86, 0x1A, 0x16, 0x68, 0x0F, 0x3A, 0xD9, 0xC0, 0x2D, 0xCC,
                     0xEF,   0x3C, 0xC1, 0xFA, 0x3C, 0xDB, 0xE4, 0xCE, 0x6D, 0x54, 0xB8, 0x0D, 0xEA, 0xC1, 0xBC, 0x21 };
    mip     = mirsys( 1000, 16 );
    mip->IOBASE = 16;
    x       = mirvar( 0 );
    y       = mirvar( 0 );
    kG      = epoint_init();
    p_x_char_out = malloc(SM2_NUMWORD);
    HexString2BCD(pub_key_x, SM2_NUMWORD*2, p_x_char_out, SM2_NUMWORD);
    p_y_char_out = malloc(SM2_NUMWORD);
    HexString2BCD(pub_key_y, SM2_NUMWORD*2, p_y_char_out, SM2_NUMWORD);

    bytes_to_big( 32, p_x_char_out, x );
    bytes_to_big( 32, p_y_char_out, y );
...
/* initiate SM2 curve */
	...
	// encrypt
	SM2_Encrypt(std_rand, kG, data, data_len, cipher);
    free(p_x_char_out);
    free(p_y_char_out);
}


int SM2_Decrypt_New(unsigned char *cipher, int cipher_len, unsigned char *private_key, unsigned char *data)
{
    unsigned char *private_key_byte = NULL;
    unsigned char *cipher_byte = NULL;
    big     ks;
    int cipher_byte_len = 0;
    mip     = mirsys( 1000, 16 );
    mip->IOBASE = 16;
    ks = mirvar( 0 );

    private_key_byte = malloc(SM2_NUMWORD);
    HexString2BCD(private_key, SM2_NUMWORD*2, private_key_byte, SM2_NUMWORD);
    bytes_to_big(SM2_NUMWORD, private_key_byte, ks ); /* ks is the standard private key */

    cipher_byte_len = cipher_len/2;
    cipher_byte = malloc(cipher_byte_len);
    HexString2BCD(cipher, cipher_len, cipher_byte, cipher_byte_len);
printf("cipher-byte:%s,pkey:%s\n", cipher_byte, private_key_byte);
    SM2_Init();
    SM2_Decrypt( ks, cipher_byte, cipher_byte_len, data);
    printf("sm2-de-data:%s\n", data);
}
